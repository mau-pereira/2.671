function [] = improvePlot()
    ...
    % Keep renderer settings, but do not force figure Position here.
    % Caller controls final window size/aspect.
    set(hFig, 'Renderer', 'painters');

    % Grab the axes handle(s)
    axis_handles = findobj(hFig, 'type', 'axes');
...
%             LineH(i).Color = [0 0 0]; % Set all error bars to black
    ...
    for k = 1:length(h)
        if strcmpi(get(h(k),'Tag'),'legend')
            % Keep current legend location selected by caller.
